...
# CV with bagging
set.seed(12345678)
WAUS.bag.cv <- bagging.cv(RainTomorrow~., data = WAUS.train, v = 10, mfinal = 100, control = rpart.control(cp=0.01))
WAUS.bag.cv$confusion
(WAUS.bag.cv$confusion[1]+WAUS.bag.cv$confusion[4])/sum(WAUS.bag.cv$confusion)

#------------------------------------------------------------------------------------------------------
# CV with boosting
set.seed(12345678)
WAUS.boost.cv <- boosting.cv(RainTomorrow~., data = WAUS.train, v = 10, boos = TRUE, 
                             mfinal = 100, coeflearn = "Breiman", control = rpart.control(cp = 0.01))
WAUS.boost.cv$confusion
(WAUS.boost.cv$confusion[1]+WAUS.boost.cv$confusion[4])/sum(WAUS.boost.cv$confusion)
